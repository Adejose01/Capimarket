...
export default function StoreCatalogView() {
  ...
  const availableCategories = [
    "Todos",
    ...new Set(products.map((p) => p.expand?.category?.name).filter(Boolean)),
  ];
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(
      (p) =>
        ...
        p.expand?.category?.name === activeCategory,
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-slate-200">
      <div className="h-48 md:h-64 w-full bg-slate-900 overflow-hidden relative">
        {store.banner ? (
          <img
            src={getImageUrl(store, store.banner, "1200x400")}
            alt="Banner"
        ...
        <button
          onClick={() => navigate("/")}
          className="absolute top-4 left-4 md:top-8 md:left-8 flex items-center gap-2 text-xs font-bold uppercase tracking-widest bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-full border border-white/20 transition-all backdrop-blur-md z-20"
        >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-6 -mt-16 md:-mt-20 mb-8">
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-white border-4 border-white shadow-xl overflow-hidden shrink-0 z-20">
            {store.logo ? (
              <img
                src={getImageUrl(store, store.logo, "200x200")}
                alt={store.name}
