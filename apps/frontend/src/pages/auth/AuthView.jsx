...
export default function AuthView() {
  ...
  const { login, register, loginWithGoogle, isAuthenticated, isLoading } =
    useAuthStore();
    ...
    if (isLogin) {
      ...
      if (result.success) {
        ...
        if (pbError.name) {
          ...
          toast.error(
            "Error en la confirmación: " + pbError.passwordConfirm.message,
          );
        } else {
          toast.error(
            "⚠️ Error del servidor: " + JSON.stringify(result.error?.response),
          );
        }
      }
    }
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      console.log(error);
      toast.error(
        "Error al enviar el correo. Verifica que el correo sea válido.",
      );
    } finally {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-[#0a0a0a] p-8 md:p-10 rounded-3xl border border-slate-100 shadow-xl relative overflow-hidden">
          ...
          <p className="text-sm font-medium text-slate-500 mb-8">
            {isLogin
              ? "Ingresa tus datos para continuar en CapiMercado."
              : "Únete al ecosistema de tecnología más grande."}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-slate-900 transition-all font-medium"
                />
              </div>
            )}
            <div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-slate-900 text-white py-4 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-slate-800 transition-all mt-4 disabled:opacity-50 shadow-md"
            >
              {isLoading
                ? "Cargando..."
                : isLogin
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M5.50254 14.3003C5.25254 13.5545 5.12012 12.7665 5.12012 11.9545C5.12012 11.1425 5.25254 10.3545 5.50254 9.60875V6.51782H1.51659C0.666418 8.21003 0.179199 10.0381 0.179199 11.9545C0.179199 13.8709 0.666418 15.6989 1.51659 17.3911L5.50254 14.3003Z"
                fill="#FBBC05"
              />
              <path
                d="M12.2401 4.74966C13.999 4.72102 15.6946 5.38557 16.9431 6.59711L20.2739 3.26629C18.1561 1.27211 15.2536 0.172465 12.2401 0.200877C7.7029 0.200877 3.55371 2.75831 1.5166 6.81048L5.50255 9.90141C6.45055 7.06175 9.10947 4.94966 12.2401 4.74966Z"
                fill="#EA4335"
              />
            </svg>
            Google
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-bold text-slate-900 hover:underline px-4 py-2 rounded-lg hover:bg-slate-50 transition-colors"
            >
