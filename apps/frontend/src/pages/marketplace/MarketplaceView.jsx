...
export default function MarketplaceView({ exclusiveStoreId = null, exclusiveStoreSlug = null }) {
  ...
  useEffect(() => {
    ...
    loadProducts();
  }, [currentPage, debouncedSearchTerm, activeStore, activeCategory, minPrice, maxPrice, filterCond, filterLoc, sortOrder, exclusiveStoreId, categories]);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#050505] text-slate-900 dark:text-white font-sans selection:bg-slate-200 flex flex-col">
      ...
      <motion.main initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 180, damping: 22 }} className="grow" style={{ willChange: "transform" }}>
        {isInitialLoading ? (
              ...
              <div className="relative z-10 px-8 pt-24 pb-10 md:px-16 md:pt-28 w-full max-w-3xl text-white flex flex-col items-center md:items-start">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-2 leading-tight tracking-tight text-center md:text-left flex items-center justify-center md:justify-start gap-3">
                  ...
                  {activeStore.instagram && (
                    <a href={`https://instagram.com/${activeStore.instagram.replace('@', '')}`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-linear-to-tr from-[#fd5949] to-[#d6249f] hover:scale-110 transition-transform shadow-lg">
