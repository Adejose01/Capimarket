...
export default function ProductFormModal({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (product) {
        await ProductsService.update(product.id, productPayload);
  ...
  const content = (
    <div
      className={`bg-white ${
        isModal
          ? "rounded-4xl p-8 md:p-10 w-full max-w-2xl shadow-2xl relative max-h-[90vh] overflow-y-auto no-scrollbar border border-slate-100"
          : "rounded-4xl p-8 border border-slate-100 max-w-4xl mx-auto md:p-12 shadow-premium animate-in fade-in duration-300"
      }`}
    >
      {isModal && (
        <button
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <h3 className="font-extrabold text-2xl tracking-tight text-slate-900">
          {product ? "Editar Publicación" : "Nuevo Artículo"}
        </h3>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div>
            ...
            <select
              name="listed"
              defaultValue={product ? String(product.listed) : "true"}
              required
            ...
            >
              <option value="true">👁️ Publicado</option>
          ...
          {product && product.images && product.images.length > 0 && (
            <div className="mb-5">
              <label className="text-xs font-bold text-slate-500 mb-3 block">
                Fotos actuales ({product.images.length - imagesToDelete.length}{" "}
                activas)
              </label>
        ...
        <div className="pt-6 flex flex-col md:flex-row gap-4 justify-end">
          {isModal && (
            <button
              type="button"
              onClick={onClose}
              className="px-8 py-4 bg-white text-slate-500 border border-slate-200 rounded-full text-sm font-bold hover:bg-slate-50 transition-colors cursor-pointer text-center"
            >
              Cancelar
            </button>
          )}
