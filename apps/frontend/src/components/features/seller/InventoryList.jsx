      ...
      {inventory.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {inventory?.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`bg-white p-4 rounded-3xl border border-slate-100 flex items-center gap-4 group transition-all shadow-sm hover:shadow-premium hover:-translate-y-1 ${p.stock === "out_of_stock" ? "opacity-60 grayscale" : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2 max-w-full overflow-hidden">
                    {p.condition && (
                      <span className="text-[9px] uppercase tracking-widest bg-slate-100 px-2 py-1 shrink-0 rounded-md text-slate-600 font-semibold">
                        {p.condition === "new"
                          ? "✨ Nuevo"
                    ...
                    {p.stock === "out_of_stock" && (
                      <span className="text-[9px] uppercase tracking-widest bg-red-50 text-red-500 px-2 py-1 rounded-md font-bold shrink-0">
                        ❌ Agotado
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  <button
                    ...
                    className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-slate-900 hover:text-white transition-colors"
                  >
                    <Edit2 size={16} />
                  </button>
                  <button
                    onClick={() => handleDelete(p.id)}
                    className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:bg-red-500 hover:text-white transition-colors"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
        ...
        <div className="py-20 flex flex-col items-center justify-center text-center border-2 border-dashed border-slate-200 rounded-[40px] bg-white text-slate-400">
          ...
          <button
            ...
            className="px-8 py-3 bg-slate-900 text-white font-bold text-sm rounded-full hover:bg-slate-800 transition-colors shadow-premium flex items-center gap-2 hover:-translate-y-0.5"
          >
