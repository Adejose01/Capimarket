...
export default function ProductCard({
  ...
  getImageUrl: (record: any, filename: string, thumb?: string) => string | undefined;
  ...
  const comprarPorWhatsAppRapido = (e: React.MouseEvent, producto: ProductRecord) => {
    e.stopPropagation();
  ...
  return (
    <motion.div
      ...
      onClick={() =>
        navigate(`/producto/${product.id}`, { state: { background: location } })
      }
    >
      <div className="relative aspect-square overflow-hidden bg-slate-50 dark:bg-[#111] p-6 flex flex-col items-center justify-center">
        {mainImage ? (
          ...
          <div className="w-[90%] h-[90%] flex items-center justify-center bg-slate-200 dark:bg-slate-800 rounded-xl text-slate-400">
            Sin Imagen
          </div>
        )}
        ...
        {isOutOfStock && (
          <div className="absolute inset-0 bg-white/40 dark:bg-black/40 backdrop-blur-[2px] z-20 flex items-center justify-center">
            <span className="bg-white/90 dark:bg-black/90 backdrop-blur-md border border-slate-200/50 dark:border-white/10 text-slate-900 dark:text-white font-extrabold px-3 py-1 sm:px-4 sm:py-1.5 rounded-full uppercase tracking-widest text-[9px] sm:text-[10px] shadow-sm">
              Agotado
            </span>
          </div>
        )}
        ...
        <div className="absolute top-2 right-2 sm:top-3 sm:right-3 flex flex-col gap-1 sm:gap-2 items-end z-30">
          <span className="text-[8px] sm:text-[9px] font-extrabold px-2 py-1 rounded-full uppercase tracking-widest bg-white/90 dark:bg-[#1a1a1a]/90 text-slate-800 dark:text-slate-200 backdrop-blur-md shadow-sm border border-slate-200/50 dark:border-white/10 max-w-90px truncate">
            {activeStoreName
              ? activeStoreName
              : product.expand?.store?.name || "Stock"}
          </span>
          {displayCondition && (
      ...
      <div className="p-4 flex-1 flex flex-col z-10 bg-white dark:bg-[#0a0a0a]">
        ...
        <h3 className="text-sm sm:text-lg font-extrabold text-slate-900 dark:text-white mb-2 line-clamp-2 leading-tight tracking-tight">
          {product.name}
        </h3>
        ...
        <div className="flex items-end justify-between mt-auto pt-3 sm:pt-4 border-t border-slate-50 dark:border-white/5">
          ...
          <button
            ...
            className="bg-[#25D366] dark:bg-white text-white dark:text-slate-900 p-2 sm:p-2.5 rounded-full transition-all shadow-sm hover:scale-110 active:scale-95 disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-1 group/btn"
          >
