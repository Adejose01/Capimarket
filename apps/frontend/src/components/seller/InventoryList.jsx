              ...
              <div key={p.id} className={`bg-white p-4 rounded-3xl border border-slate-100 flex items-center gap-4 group transition-all shadow-sm hover:shadow-premium hover:-translate-y-1 ${p.stock === 'out_of_stock' ? 'opacity-60 grayscale' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2 max-w-full overflow-hidden">
                    {p.condition && <span className="text-[9px] uppercase tracking-widest bg-slate-100 px-2 py-1 shrink-0 rounded-md text-slate-600 font-semibold">{p.condition === 'new' ? '✨ Nuevo' : (p.condition === 'open_box' ? '📂 Open Box' : '📦 Usado')}</span>}
                    {p.stock === 'out_of_stock' && <span className="text-[9px] uppercase tracking-widest bg-red-50 text-red-500 px-2 py-1 rounded-md font-bold shrink-0">❌ Agotado</span>}
                  </div>
                </div>
