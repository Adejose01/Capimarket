...
- Corregir el evento de clic (_tap/click_) sobre las imágenes principales del producto para que se abran limpiamente en una nueva pestaña del navegador.
...
### ▢ Refactorizar y corregir el Footer

- Reestructurar el mapa del sitio en el pie de página bajo la siguiente jerarquía:
  - **Compañía:** `Sobre nosotros`, `Sedes`, `ADJML LLC`.
  - **Soporte:** `Garantía Premium`, `Métodos de Envío`, `Preguntas Frecuentes`.
  - **Enlaces directos:** `Newsletter`, `Privacidad`, `Términos`, `Instagram`.
- **Comportamiento:** Asegurar que al interactuar con cualquiera de estos enlaces se ejecute un desplazamiento automático al inicio de la página (_scroll to top_).
